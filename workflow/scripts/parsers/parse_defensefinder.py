...
def parse_defensefinder(genes_tsv, systems_tsv, hmmer_tsv, faa_path, genome_id):
    ...
    for feat_type in ['cas_gene', 'rm_gene', 'defense_gene', 'antidefense_gene']:
        ...
        base_cols = ['genome_id', 'feature_type', 'ID', 'Parent', 'contig', 'start', 'end', 
                     'strand', 'score', 'gene_name', 'prodigal_id']
        cols = [c for c in base_cols if c in type_df.columns]
        result[feat_type] = type_df[cols].reset_index(drop=True)
...
def main(snakemake):
    ...
    # Input names match the rule's input definition
    genes_tsv = snakemake.input.genes
    systems_tsv = snakemake.input.systems
    hmmer_tsv = snakemake.input.hmmer
    ...
    cas_systems.to_csv(snakemake.output.cas_systems, sep='\t', index=False, na_rep='NULL')
    rm_systems.to_csv(snakemake.output.rm_systems, sep='\t', index=False, na_rep='NULL')
    defense_systems.to_csv(snakemake.output.defense_systems, sep='\t', index=False, na_rep='NULL')
    antidefense_systems.to_csv(snakemake.output.antidefense_systems, sep='\t', index=False, na_rep='NULL')
    ...
    cas_genes.to_csv(snakemake.output.cas_genes, sep='\t', index=False, na_rep='NULL')
    rm_genes.to_csv(snakemake.output.rm_genes, sep='\t', index=False, na_rep='NULL')
    defense_genes.to_csv(snakemake.output.defense_genes, sep='\t', index=False, na_rep='NULL')
    antidefense_genes.to_csv(snakemake.output.antidefense_genes, sep='\t', index=False, na_rep='NULL')
