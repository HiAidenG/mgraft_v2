...
def merge_alignments(rm_genes_df, alignments_tsv):
    ...
    # Read diamond output - handle variable number of fields due to tabs in stitle
    try:
        aln_df = pd.read_csv(
            alignments_tsv,
            sep='\t',
            ...
            names=['qseqid', 'sseqid', 'pident', 'length', 'qlen', 'slen',
                   'qcovhsp', 'scovhsp', 'evalue', 'bitscore', 'stitle'],
            on_bad_lines='warn',  # Skip lines with wrong number of fields
            engine='python'  # More forgiving parser
        )
    except Exception as e:
        print(f"Warning: Could not parse alignments: {e}", file=sys.stderr)
        rm_genes_df = rm_genes_df.copy()
        rm_genes_df['REBASE_accession'] = None
        rm_genes_df['REBASE_protein_id'] = None
        rm_genes_df['identity'] = None
    ...
    # Ensure tool_id is string type for merge
    rm_genes_df = rm_genes_df.copy()
    rm_genes_df['tool_id'] = rm_genes_df['tool_id'].astype(str)
    aln_df['tool_id'] = aln_df['tool_id'].astype(str)
...
def main(snakemake):
    ...
    # Outputs - match rule output names
    rm_genes_out = snakemake.output.rm_genes
    rm_systems_out = snakemake.output.rm_systems
