...
def parse_crisprdetect_txt(txt_path: str, genome_id: str) -> Tuple[List[dict], List[dict]]:
    ...
    for block in blocks:
        ...
        # Format: >NC_000913.3 Escherichia coli str. K-12 substr. MG1655, complete genome         Array_Orientation: Reverse
        # We only want the first word (contig ID)
        ...
        # Format: ID_START_STOP_DIR: NC_000913.3 Escherichia coli ...-2877701-2878464-R; REP_DR:...; ...
        # The coordinates are at the END of the ID field, separated by dashes
        summary_match = re.search(
            r'ID_START_STOP_DIR:\s*(.+?)-(\d+)-(\d+)-([FR]);'
            r'\s*REP_DR:([^;]+);'
            r'\s*DR_LENGTH:(\d+);'
            r'\s*NUM_DRs:(\d+);'
            r'.*?SCORE:([\d.]+);'
        ...
        # Extract spacer coordinates from the position table in the TXT block
        # The table has format:
        #   Position	Repeat	   %id	Spacer	Repeat_Sequence	Spacer_Sequence
        #       1028	    23	  95.7	    20	...             ACGGCTGT...
        # Position = GENOMIC coordinate of repeat start (already absolute, not relative)
        # Repeat = repeat length, Spacer = spacer length
        # Spacer starts at (repeat_position + repeat_length)
        position_table_match = re.search(
            r'Position\s+Repeat\s+%id\s+Spacer.*?=+\n(.*?)\n=+',
            block, re.DOTALL
        )
        spacer_coordinates = []
        if position_table_match:
            table_text = position_table_match.group(1)
            for line in table_text.strip().split('\n'):
                # Parse each row: Position, Repeat_len, %id, Spacer_len, ...
                parts = line.split()
                if len(parts) >= 4:
                    try:
                        repeat_pos = int(parts[0])  # Already genomic coordinate
                        repeat_len = int(parts[1])
                        spacer_len = int(parts[3])
                        if spacer_len > 0:  # Skip terminal repeat (spacer_len = 0)
                            # Spacer starts immediately after repeat ends
                            spacer_start = repeat_pos + repeat_len
                            spacer_end = spacer_start + spacer_len - 1
                            spacer_coordinates.append((spacer_start, spacer_end))
        ...
        arrays.append({
            ...
            'cas_strand': None,
            '_spacer_coordinates': spacer_coordinates  # Internal: used for spacer coordinate assignment
        })
        
        # Create spacer entries with coordinates from position table
        ...
        for i, spacer_seq in enumerate(spacer_seqs, start=1):
            # Get coordinates from pre-parsed position table
            spacer_start = None
            spacer_end = None
            if i <= len(spacer_coordinates):
                spacer_start, spacer_end = spacer_coordinates[i - 1]
...
def get_empty_arrays_df() -> pd.DataFrame:
    ...
    return pd.DataFrame(columns=[
        ...
        'quality_score', 'questionable_array', 'direction_confidence', 'cas_strand'
    ])
...
def get_empty_spacers_df() -> pd.DataFrame:
    ...
    return pd.DataFrame(columns=[
        'genome_id', 'feature_type', 'ID', 'Parent', 'contig', 'start', 'end', 'strand',
        'tool_id', 'spacer_idx', 'sequence', 'length'
    ])
...
def main(snakemake):
    ...
    txt_path = snakemake.input.txt
    genome_id = snakemake.params.genome_id
    arrays_tsv = snakemake.output.arrays
    spacers_tsv = snakemake.output.spacers
    ...
    # Get spacer coordinates from GFF as fallback for spacers missing coordinates
    # (TXT parsing now extracts coordinates directly, but GFF is backup)
    spacer_coords = parse_spacer_coordinates_from_gff(gff_path)
    
    # Assign coordinates to spacers from GFF data ONLY if TXT parsing didn't provide them
    for spacer in spacers_list:
        if spacer['start'] is not None and spacer['end'] is not None:
            continue  # Already have coordinates from TXT parsing
